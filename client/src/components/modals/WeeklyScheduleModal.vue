            ...
            <tbody>
              <tr v-for="period in periods" :key="period">
                ...
                <td v-for="day in daysOfWeek" :key="`${day}-${period}`">
                  <select v-model="editableSchedule[day][period - 1]" class="form-control form-control-sm period-select"
                    :disabled="isSaving">
                    ...
                    <option v-for="cls in availableClasses" :key="cls.id" :value="cls.id">
                      ...
                      <template v-else-if="cls.classType === 'special'">
                        {{ cls.className }} <span v-if="cls.yearLevel" class="special-year-level-option">(Yr {{
                          cls.yearLevel }})</span>
                      </template>
                      <template v-else>
                        {{ cls.id }} </template>
...
// --- Component State ---
const daysOfWeek = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday']; // Days to display in the grid
...
.modal-content.large {
  /* Class to make the modal wider */
  min-width: 80vw;
  /* Example: Use 80% of viewport width */
  max-width: 1200px;
  /* Set a maximum width */
}
...
.footer-error {
  /* Style for error message in footer */
  margin-right: auto;
  /* Push error to the left */
  margin-bottom: 0;
...
.schedule-grid-table {
  ...
  table-layout: fixed;
  /* Helps keep column widths consistent */
}
...
.schedule-grid-table td {
  ...
  padding: 0.3rem;
  /* Reduced padding for tighter grid */
  text-align: center;
  vertical-align: middle;
  font-size: 0.85rem;
  min-width: 100px;
  /* Minimum width for each day column */
}
...
.period-select {
  ...
  /* Try to force dropdown appearance */
