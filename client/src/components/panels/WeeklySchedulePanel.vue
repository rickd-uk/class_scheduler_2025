<template>
  <div class="panel weekly-schedule-panel">
    ...
    <div
      v-else-if="hasScheduleData && !isLoadingClasses"
        ...
        <tbody>
          <tr v-for="period in periods" :key="period">
            ...
            <td
              ...
              :key="`${day}-${period}`"
              :style="{
                backgroundColor: getClassColor(day, period) || 'transparent',
              }"
            >
              <div v-if="getClassForPeriod(day, period)" class="schedule-item">
                <span
                  v-if="getClassForPeriod(day, period).classType === 'numbered'"
                >
                  {{ formatNumberedClassName(getClassForPeriod(day, period)) }}
                </span>
                <span
                  v-else-if="
                    getClassForPeriod(day, period).classType === 'special'
                  "
                >
                  {{ getClassForPeriod(day, period).className }}
                  <span
                    v-if="getClassForPeriod(day, period).yearLevel"
                    class="special-year-level-display"
                  >
    ...
    <p v-else class="placeholder-content">
      No regular weekly schedule defined yet, or class data is loading. Click
      'Edit Schedule' to create one.
    </p>
  </div>
</template>
...
const hasScheduleData = computed(() => {
  return (
    ...
    daysOfWeek.some((day) =>
      scheduleData.value[day]?.some((slot) => slot !== null),
    )
  );
});
...
const getClassForPeriod = (day, period) => {
  ...
  if (!classes.value || classes.value.length === 0) {
    console.warn(
      `[WeeklySchedulePanel] getClassForPeriod: Classes not loaded yet for classId ${classIdToFind}`,
    );
    return null; // Or a placeholder indicating classes are loading
  }
  const foundClass = classes.value.find(
    (cls) => String(cls.id) === String(classIdToFind),
  );
