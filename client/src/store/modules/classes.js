  ...
  mutations: {
    ...
    SET_CLASSES(state, classes) {
      state.classes = classes
        ...
        .sort((a, b) => {
          // Sort logic matching backend order
          if (a.classType !== b.classType)
            return a.classType.localeCompare(b.classType); // numbered before special
          if (a.classType === "numbered") {
            const yearCompare = String(a.yearLevel).localeCompare(
              String(b.yearLevel),
            );
    ...
    UPDATE_CLASS(state, updatedClass) {
      ...
      if (index !== -1) {
        ...
        console.warn(
          `Could not find class with ID ${updatedClass.id} to update in state.`,
        );
      }
    },
  ...
  actions: {
    ...
    async addClass({ commit, dispatch }, classData) {
      ...
      try {
        ...
        dispatch(
          "ui/showNotification",
          { type: "success", message: "Class added successfully!" },
          { root: true },
        );
    ...
    async updateClass({ commit, dispatch }, { id, data }) {
      ...
      try {
        console.log(
          `[updateClass Action] Updating class ID ${id} with data:`,
          data,
        );
        ...
        dispatch(
          "ui/showNotification",
          { type: "success", message: "Class updated successfully!" },
          { root: true },
        );
    ...
    async deleteClass({ commit, dispatch }, classId) {
      ...
      try {
        ...
        dispatch(
          "ui/showNotification",
          { type: "success", message: "Class deleted successfully." },
          { root: true },
        );
      } catch (error) {
        const message =
    ...
    async linkTextbook({ commit, dispatch }, { classId, textbookId }) {
      ...
      try {
        ...
        dispatch(
          "ui/showNotification",
          { type: "success", message: "Textbook linked!" },
          { root: true },
        );
      } catch (error) {
        const message =
    ...
    async unlinkTextbook(
      ...
      try {
        ...
        const currentClass = rootState.classes.classes.find(
          (c) => c.id === classId,
        );
        if (currentClass) {
          const updatedTextbooks = currentClass.textbooks.filter(
            (tb) => tb.id !== textbookId,
          );
          commit("UPDATE_CLASS_TEXTBOOKS", {
            classId: classId,
          ...
          console.warn(
            "Could not find class locally to update after unlinking textbook.",
          );
        }
        console.log(`Unlinked textbook ${textbookId} from class ${classId}`);
        dispatch(
          "ui/showNotification",
          { type: "success", message: "Textbook unlinked." },
          { root: true },
        );
      } catch (error) {
        const message =
