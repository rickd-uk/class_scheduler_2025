  ...
  mutations: {
    ...
    UPDATE_PATTERN(state, updatedPattern) {
      ...
      if (index !== -1) {
        ...
        console.warn(`Could not find pattern with ID ${updatedPattern.id} to update in state.`);
      }
    },
  ...
  actions: {
    ...
    async fetchPatterns({ commit, state }) {
      ...
      try {
        ...
        const message = error.response?.data?.message || error.message || 'Failed to fetch exception patterns';
    ...
    async addPattern({ commit, dispatch }, patternData) {
      ...
      try {
        ...
        dispatch('ui/showNotification', { type: 'success', message: 'Exception pattern created!' }, { root: true });
        return response.data; // Return created pattern data
      } catch (error) {
        // Handle errors during add
        const message = error.response?.data?.message || error.message || 'Failed to create pattern';
    ...
    async updatePattern({ commit, dispatch }, { id, data }) {
      ...
      try {
        ...
        dispatch('ui/showNotification', { type: 'success', message: 'Exception pattern updated!' }, { root: true });
        return response.data; // Return updated pattern data
      } catch (error) {
        // Handle errors during update
        const message = error.response?.data?.message || error.message || 'Failed to update pattern';
    ...
    async deletePattern({ commit, dispatch }, patternId) {
      ...
      try {
        ...
        // Dispatch success notification
        dispatch('ui/showNotification', { type: 'success', message: 'Exception pattern deleted.' }, { root: true });
      } catch (error) {
        // Handle errors during delete
        const message = error.response?.data?.message || error.message || 'Failed to delete pattern';
        // Dispatch error notification
        dispatch('ui/showNotification', { type: 'error', message }, { root: true });
        throw new Error(message); // Re-throw error
      }
