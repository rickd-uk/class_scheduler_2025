...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      ...
      console.log(
        "Modifying classNumber column to allow NULL (if necessary)...",
      );
      ...
      try {
        ...
        const allConstraints = await queryInterface.getConstraints(tableName, {
          transaction,
        });
        constraintExists = allConstraints.some(
          (c) =>
            c.constraintName === constraintName &&
            c.constraintType === "UNIQUE",
        );
        if (constraintExists) {
          // Ensured clear formatting
          console.log(
            `Unique constraint ${constraintName} already exists (found via getConstraints). Skipping add.`,
          );
        }
      } catch (e) {
        console.warn(
          `Could not definitively check for constraint ${constraintName} using getConstraints. Will attempt to add. Error: ${e.message}`,
        );
      }
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      console.log(
        `Removing unique constraint ${constraintName} (if it exists)...`,
      );
      await queryInterface.removeConstraint(tableName, constraintName, {
        transaction,
      });
      console.log(
        `Unique constraint ${constraintName} removed (if it existed).`,
      );
      ...
      console.log(
        "Reverting classNumber column (setting allowNull to false)...",
      );
      await queryInterface.changeColumn(
        tableName,
        "classNumber",
      ...
      await queryInterface.removeColumn(tableName, "className", {
        transaction,
      });
      console.log("className column removed (if it existed).");

      console.log("Removing classType column (if it exists)...");
      await queryInterface.removeColumn(tableName, "classType", {
        transaction,
      });
