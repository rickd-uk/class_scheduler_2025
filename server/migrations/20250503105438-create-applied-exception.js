...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      console.log(`[Migration 20250503105438] Starting migration for ${tableName}...`);
      let tableExists = false;
      try {
        await queryInterface.describeTable(tableName, { transaction });
        tableExists = true;
        console.log(`[Migration 20250503105438] Table ${tableName} already exists.`);
      } catch (error) {
        // If describeTable throws an error, it usually means the table doesn't exist.
        console.log(`[Migration 20250503105438] Table ${tableName} does not exist. Creating it...`);
      }
      ...
      try {
        ...
        constraintExists = constraints.some(c => c.constraintName === constraintName && c.constraintType === 'UNIQUE');
      } catch (e) {
          console.warn(`[Migration 20250503105438] Could not reliably check for constraint ${constraintName}. Error: ${e.message}. Will attempt to add.`);
      }


      if (!constraintExists) {
        console.log(`[Migration 20250503105438] Adding unique constraint ${constraintName} to ${tableName}...`);
        ...
        console.log(`[Migration 20250503105438] Unique constraint ${constraintName} added.`);
      } else {
        console.log(`[Migration 20250503105438] Unique constraint ${constraintName} already exists on ${tableName}. Skipping add.`);
      }
      ...
      indexExists = indexes.some(idx => {
          const idxFields = idx.fields.map(f => f.attribute).sort().join(',');
      ...
      if (!indexExists) {
        console.log(`[Migration 20250503105438] Adding index on ${indexFields.join(', ')} to ${tableName}...`);
        ...
        const newIndexes = await queryInterface.showIndex(tableName, { transaction });
        ...
        console.log(`[Migration 20250503105438] Index on ${indexFields.join(', ')} added (name: ${indexNameToPossiblyRemoveInDown || 'auto-generated'}).`);
      } else {
        console.log(`[Migration 20250503105438] An index on ${indexFields.join(', ')} already exists on ${tableName} (possibly from unique constraint). Skipping addIndex.`);
      }

      await transaction.commit();
      console.log(`[Migration 20250503105438] Up migration for ${tableName} successful.`);
    } catch (err) {
      console.error(`[Migration 20250503105438] Up migration for ${tableName} failed:`, err);
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      console.log(`[Migration 20250503105438] Reverting migration for ${tableName}...`);

      console.log(`[Migration 20250503105438] Removing unique constraint ${constraintName} from ${tableName} (if it exists)...`);
      await queryInterface.removeConstraint(tableName, constraintName, { transaction });
      console.log(`[Migration 20250503105438] Unique constraint ${constraintName} removed.`);
      ...
      console.log(`[Migration 20250503105438] Removing index on ${indexFields.join(', ')} from ${tableName} (if it exists)...`);
      ...
      try {
        ...
        if (foundIndex) {
            await queryInterface.removeIndex(tableName, foundIndex.name, { transaction });
            console.log(`[Migration 20250503105438] Index ${foundIndex.name} removed.`);
        } else {
            console.log(`[Migration 20250503105438] No specific index found for fields ${indexFields.join(', ')} to remove separately.`);
        }
      } catch (e) {
          console.warn(`[Migration 20250503105438] Could not robustly remove index by fields. It might have been removed with the constraint, or you might need to specify the name. Error: ${e.message}`);
      }
      ...
      await transaction.commit();
      console.log(`[Migration 20250503105438] Down migration for ${tableName} successful.`);
    } catch (err) {
      console.error(`[Migration 20250503105438] Down migration for ${tableName} failed:`, err);
      await transaction.rollback();
      throw err;
    }
  }
