...
router.get("/", authenticateToken, async (req, res) => {
  ...
  try {
    console.log(
      `[GET /api/applied-exceptions] Fetching all applied exceptions for user ${userId}`,
    );
    ...
    console.log(
      `[GET /api/applied-exceptions] Found ${applied.length} exceptions for user ${userId}`,
    );
    res.status(200).json(applied); // Send the array of exceptions
  } catch (error) {
    console.error(
      `Error fetching all applied exceptions for user ${userId}:`,
      error,
    );
    res.status(500).json({
      message: "Error fetching applied exceptions",
      error: error.message,
    });
  }
});
...
router.get("/:date", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Error fetching applied exception",
      error: error.message,
    });
  }
});
...
router.post("/", authenticateToken, async (req, res) => {
  ...
  if (isDayOff && exceptionPatternId) {
    return res.status(400).json({
      message: "Cannot apply a pattern and mark as day off simultaneously.",
    });
  }
  ...
  try {
    ...
    if (!created) {
      ...
      console.log(
        `Updating existing applied exception for ${date}, user ${userId}`,
      );
      appliedException.exceptionPatternId = isDayOff
        ? null
        : exceptionPatternId;
      appliedException.isDayOff = isDayOff || false;
      appliedException.reason = reason !== undefined ? reason : null; // Allow clearing reason
      appliedException.color = isDayOff
        ? color !== undefined
          ? color
          : "#F0F0F0"
        : null; // Update/clear color only if day off
    ...
    console.error(
      `Error applying exception for user ${userId} on date ${date}:`,
      error,
    );
    if (error.name === "SequelizeForeignKeyConstraintError") {
      return res
        .status(400)
        .json({ message: "Invalid Exception Pattern ID provided." });
    }
    res
      .status(500)
      .json({ message: "Error applying exception", error: error.message });
  }
});
...
router.delete("/:date", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (result === 0) {
      return res
        .status(404)
        .json({ message: "No applied exception found for this date." });
    }
    res.status(204).send();
  } catch (error) {
    res.status(500).json({
      message: "Error clearing applied exception",
      error: error.message,
    });
  }
});
