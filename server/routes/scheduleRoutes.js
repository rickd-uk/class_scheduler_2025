...
router.get("/regular", authenticateToken, async (req, res) => {
  const userId = req.user.id;
  try {
    console.log(
      `[GET /api/schedule/regular] Fetching schedule for user ID: ${userId}`,
    );
    // Fetch the full user object initially to ensure schedule data is loaded if present
    const user = await User.findByPk(userId);
    if (!user) {
      console.error(
        `[GET /api/schedule/regular] User not found for ID: ${userId}`,
      );
      return res.status(404).json({ message: "User not found" });
    }
    console.log(
      `[GET /api/schedule/regular] Raw schedule data from DB for user ${userId}:`,
      user.regularScheduleData,
    );
...
router.put("/regular", authenticateToken, async (req, res) => {
  const userId = req.user.id;
  ...
  console.log(
    `[PUT /api/schedule/regular] Received schedule update for user ${userId}:`,
  );
  ...
  try {
    ...
    console.log(
      `[PUT /api/schedule/regular] User found. Data BEFORE assignment:`,
      user.regularScheduleData,
    );
    // Assign the new schedule data
    user.regularScheduleData = newSchedule;
    console.log(
      `[PUT /api/schedule/regular] Data AFTER assignment, BEFORE save:`,
      user.regularScheduleData,
    );
    ...
    try {
      console.log(
        `[PUT /api/schedule/regular] Attempting user.save() for user ${userId}...`,
      );
      await user.save(); // Attempt to save changes on the instance
      console.log(
        `[PUT /api/schedule/regular] user.save() completed WITHOUT throwing error for user ${userId}.`,
      );
    } catch (saveError) {
      console.error(
        `[PUT /api/schedule/regular] !!! ERROR during user.save() for user ${userId}:`,
        saveError,
      );
    ...
    console.log(
      `[PUT /api/schedule/regular] Data AFTER save (refetched):`,
      savedData,
    );
    // --- End Refetch ---

    console.log(
      `[PUT /api/schedule/regular] Save processing completed for user ${userId}.`,
    );
    ...
    console.error(
      `Error updating schedule for user ${userId} (outer catch):`,
      error,
    );
    res
      .status(500)
