...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Server error fetching textbooks.",
        error: error.message,
      });
  }
});
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error adding textbook.", error: error.message });
  }
});
...
router.put("/:id", async (req, res) => {
  ...
  try {
    console.log(
      `[PUT /api/textbooks] Attempting to update textbook ID: ${textbookId}`,
    );
    ...
    console.log(
      `[PUT /api/textbooks] Successfully updated textbook ID: ${textbookId}`,
    );
    res.status(200).json(textbookToUpdate); // Return the updated textbook
  } catch (error) {
    console.error(
      `[PUT /api/textbooks] Error updating textbook ID: ${textbookId}`,
      error,
    );
    if (error.name === "SequelizeValidationError") {
      const messages = error.errors.map((err) => err.message);
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    console.log(
      `[DELETE /api/textbooks] Attempting to delete textbook ID: ${textbookId}`,
    );
    const textbookToDelete = await Textbook.findByPk(textbookId);

    if (!textbookToDelete) {
      console.log(
        `[DELETE /api/textbooks] Textbook not found. ID: ${textbookId}`,
      );
      return res.status(404).json({ message: "Textbook not found." });
    }
    ...
    console.log(
      `[DELETE /api/textbooks] Successfully deleted textbook ID: ${textbookId}`,
    );
    res.status(204).send(); // No Content
  } catch (error) {
    console.error(
      `[DELETE /api/textbooks] Error deleting textbook ID: ${textbookId}`,
      error,
    );
    res
      .status(500)
